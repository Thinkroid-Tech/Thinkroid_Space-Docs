...
- **Containers** — dedicated Docker network `ts-net-<uuid>`, volumes `ts-vol-<uuid>_<type>`, container names `ts_<uuid>_<scope>_<shortId>` where `scope` ∈ `sandbox` / `deploy` / `service`. No docker object contains a display name. Labels: `ts.agent.id=<uuid>` is the identity primary key (used for every lookup); `ts.agent.name=<current-name>` is a refreshable debug metadata label. Debug workflow: run `docker ps --format` with a Go template that prints `.Names` alongside `.Label "ts.agent.name"`, or `docker inspect <name> | grep ts.agent.name`.
